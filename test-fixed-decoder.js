/**
 * Fixed BatchExecutor decoder test
 * 
 * This script tests the fixed BatchExecutor decoder that correctly
 * extracts addresses and values from the ABI-encoded calldata.
 */

// Sample executeBatch calldata for two ERC20 transfers to the same address
const TEST_CALLDATA = "";

// BatchExecutor contract on Sepolia
const BATCH_EXECUTOR_ADDRESS = '0x5dd9fdf2310b5dac8dced8a100fb4952546ae7bd';

/**
 * Helper function to convert Wei to Ether
 */
function weiToEther(wei) {
  try {
    const weiValue = BigInt(wei);
    const etherValue = Number(weiValue) / 1e18;
    return etherValue.toString();
  } catch (e) {
    return "Error converting value";
  }
}

/**
 * Helper to format address
 */
function formatAddress(address) {
  if (!address || address.length < 42) return address;
  return `${address.substring(0, 6)}...${address.substring(38)}`;
}

/**
 * Decode ERC20 transfer function
 */
function decodeERC20Transfer(calldata) {
  try {
    // Skip function selector (4 bytes/8 hex chars)
    const data = calldata.substring(10);
    
    // First parameter: to address (32 bytes/64 hex chars, padded)
    const toPadded = data.substring(0, 64);
    const to = '0x' + toPadded.substring(24);
    
    // Second parameter: amount (32 bytes/64 hex chars)
    const amountHex = data.substring(64, 128);
    const amount = BigInt('0x' + amountHex).toString();
    
    return {
      name: "transfer",
      signature: "transfer(address,uint256)",
      params: [
        { name: "_to", type: "address", value: to },
        { name: "_value", type: "uint256", value: amount }
      ]
    };
  } catch (error) {
    console.error("Failed to decode ERC20 transfer:", error);
    return null;
  }
}

/**
 * Parse the batch operations - FIXED IMPLEMENTATION
 */
function parseFixedBatchOperations() {
  try {
    // Skip the function selector (first 4 bytes/8 hex chars)
    const data = TEST_CALLDATA.substring(10);
    
    // First 32 bytes (64 hex chars) is the offset to the start of the array
    const arrayOffset = parseInt(data.substring(0, 64), 16) * 2; // Convert to nibbles
    
    // At the offset, get the array length (32 bytes / 64 hex chars)
    const arrayLengthHex = data.substring(arrayOffset, arrayOffset + 64);
    const arrayLength = parseInt(arrayLengthHex, 16);
    
    console.log(`Found ${arrayLength} operations in batch`);
    
    const operations = [];
    
    // After the array length, we have an array of offsets to each tuple
    // Each offset is 32 bytes / 64 hex chars
    const offsetsStart = arrayOffset + 64; // Start offset array after the length
    
    for (let i = 0; i < arrayLength; i++) {
      // Get the offset position for this tuple
      const offsetPosition = offsetsStart + (i * 64);
      const offsetHex = data.substring(offsetPosition, offsetPosition + 64);
      
      // Important: The offset is relative to the start of the data (after the selector)
      const tupleOffset = parseInt(offsetHex, 16) * 2; // Convert to nibbles
      
      console.log(`Operation ${i+1} offset: ${tupleOffset}`);
      
      // Start parsing tuple data at this offset
      // First 32 bytes (64 hex chars) is the address (padded to 32 bytes)
      const addressPadded = data.substring(tupleOffset, tupleOffset + 64);
      // Extract the 20-byte address from the 32-byte field (addresses are padded with zeros)
      const target = '0x' + addressPadded.substring(24);
      
      // Next 32 bytes (64 hex chars) is the value
      const valueHex = data.substring(tupleOffset + 64, tupleOffset + 128);
      const value = BigInt('0x' + valueHex).toString();
      
      // Next 32 bytes (64 hex chars) is the offset to the bytes data within this tuple
      const bytesOffsetHex = data.substring(tupleOffset + 128, tupleOffset + 192);
      const bytesOffsetInTuple = parseInt(bytesOffsetHex, 16) * 2; // Convert to nibbles
      const bytesOffset = tupleOffset + bytesOffsetInTuple; // Absolute position in data
      
      // At the bytes offset, first 32 bytes (64 hex chars) is the length of the bytes
      const bytesLengthHex = data.substring(bytesOffset, bytesOffset + 64);
      const bytesLength = parseInt(bytesLengthHex, 16) * 2; // Convert to nibbles
      
      // After the length, the actual bytes data follows
      const callData = '0x' + data.substring(bytesOffset + 64, bytesOffset + 64 + bytesLength);
      
      // Create operation object
      const operation = {
        to: target,
        value,
        callData,
        decodedCallData: null
      };
      
      console.log(`Operation ${i+1} to: ${target}, value: ${value}, calldata length: ${bytesLength/2} bytes`);
      
      // If this is an ERC20 transfer, decode it
      if (callData.startsWith('0xa9059cbb')) {
        operation.decodedCallData = decodeERC20Transfer(callData);
      }
      
      operations.push(operation);
    }
    
    return operations;
  } catch (error) {
    console.error("Error parsing operations:", error);
    return [];
  }
}

// Parse the operations with our fixed implementation
const operations = parseFixedBatchOperations();

// Display the results
console.log('\n=== FIXED BatchExecutor Operations ===\n');
operations.forEach((op, index) => {
  console.log(`Operation ${index + 1}:`);
  console.log(`  To: ${op.to} (${formatAddress(op.to)})`);
  console.log(`  Value: ${op.value} wei (${weiToEther(op.value)} ETH)`);
  console.log(`  Calldata: ${op.callData.slice(0, 20)}...${op.callData.slice(-8)}`);
  
  if (op.decodedCallData) {
    console.log('  Decoded Function:');
    console.log(`    Name: ${op.decodedCallData.name}`);
    console.log(`    Signature: ${op.decodedCallData.signature}`);
    
    op.decodedCallData.params.forEach(param => {
      let valueDisplay = param.value;
      if (param.type === 'address') {
        valueDisplay = `${param.value} (${formatAddress(param.value)})`;
      } else if (param.type === 'uint256') {
        const ethValue = weiToEther(param.value);
        valueDisplay = `${param.value} (${ethValue} ETH)`;
      }
      console.log(`    ${param.name} (${param.type}): ${valueDisplay}`);
    });
  }
  
  console.log(''); // Empty line for spacing
});

// Verify our results meet expectations
console.log('=== Verification ===');
console.log('1. Are addresses correct?', 
  operations[0]?.to === '0x5dd9fdf2310b5dac8dced8a100fb4952546ae7bd' && 
  operations[1]?.to === '0x5dd9fdf2310b5dac8dced8a100fb4952546ae7bd'
    ? 'PASS' : 'FAIL');
console.log('2. Is first transfer to the correct recipient?',
  operations[0]?.decodedCallData?.params[0]?.value === '0xbb6e6d6dabd150c4a000d1fd8a7de46a750477f4'
    ? 'PASS' : 'FAIL');
console.log('3. Is first transfer amount 1 ETH?',
  weiToEther(operations[0]?.decodedCallData?.params[1]?.value) === '1'
    ? 'PASS' : 'FAIL');
console.log('4. Is second transfer to the correct recipient?',
  operations[1]?.decodedCallData?.params[0]?.value === '0xbb6e6d6dabd150c4a000d1fd8a7de46a750477f4'
    ? 'PASS' : 'FAIL');
console.log('5. Is second transfer amount 2 ETH?',
  weiToEther(operations[1]?.decodedCallData?.params[1]?.value) === '2'
    ? 'PASS' : 'FAIL');

// Expected correct result looks like:
const EXPECTED_RESULT = {
  name: "executeBatch",
  signature: "executeBatch((address,uint256,bytes)[])",
  type: "function",
  params: [
    {
      name: "operations",
      type: "tuple[]",
      value: "array of operations",
      valueDecoded: {
        name: "operations",
        type: "tuple[]",
        signature: "operations((address,uint256,bytes)[])",
        params: [
          {
            name: "operation_0",
            type: "tuple",
            value: {
              to: "0x5dd9fdf2310b5dac8dced8a100fb4952546ae7bd",
              value: "0",
              data: "0xa9059cbb000000000000000000000000bb6e6d6dabd150c4a000d1fd8a7de46a750477f40000000000000000000000000000000000000000000000000de0b6b3a7640000"
            },
            valueDecoded: {
              name: "transfer",
              signature: "transfer(address,uint256)",
              type: "function",
              params: [
                {
                  name: "_to",
                  type: "address",
                  value: "0xbb6e6d6dabd150c4a000d1fd8a7de46a750477f4"
                },
                {
                  name: "_value",
                  type: "uint256",
                  value: "1000000000000000000" // 1 ETH
                }
              ]
            }
          },
          {
            name: "operation_1",
            type: "tuple",
            value: {
              to: "0x5dd9fdf2310b5dac8dced8a100fb4952546ae7bd",
              value: "0",
              data: "0xa9059cbb000000000000000000000000bb6e6d6dabd150c4a000d1fd8a7de46a750477f40000000000000000000000000000000000000000000000001bc16d674ec80000"
            },
            valueDecoded: {
              name: "transfer",
              signature: "transfer(address,uint256)",
              type: "function",
              params: [
                {
                  name: "_to",
                  type: "address",
                  value: "0xbb6e6d6dabd150c4a000d1fd8a7de46a750477f4"
                },
                {
                  name: "_value",
                  type: "uint256",
                  value: "2000000000000000000" // 2 ETH
                }
              ]
            }
          }
        ]
      }
    }
  ]
}; 